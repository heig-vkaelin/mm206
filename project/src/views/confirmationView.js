...
export default class CheckoutView {
  renderConfirmation(container, orderData, shippingData) {
    ...
    <div class="confirmation modal">
      ...
      orderData.record.country
    }</p>
        <h3>Order details</h3>
        ${JSON.parse(orderData.record.content)
          .map(
            (product) => `
      ...
      shippingData[orderData.record.shipping_id - 1].price
        ...
        <h3>Order total</h3>
        <p>NOK ${
          JSON.parse(orderData.record.content).reduce((acc, item) => acc + item.price * item.quantity, 0) +
          parseInt(shippingData[orderData.record.shipping_id - 1].price)
        },-</p>
